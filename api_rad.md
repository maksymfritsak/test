...
* **outlier_range (float, int, or None), default=None**: In $\sigma$. If not `None`, voxels that are not in the range [-`outlier_range` $\times\sigma$, `outlier_range` $\times\sigma$] are excluded from the intensity mask.
